...
function buildExportHtml(
  ...
  const synopsisRows = synopsisSectionMeta
    ...
    .map(({ label, key }) => `<tr><td class="label">${label}</td><td>${synopsisSections[key]}</td></tr>`)
    .join("");
  const sceneCards = scenes.map((scene, idx) => `
    <div class="scene-card">
      <div class="scene-img-wrap"><img src="${scene.imageUrl}" alt="${scene.title}" /></div>
  ...
  const summaryHtml = topSummary.filter(Boolean).length
    ? `<div class="summary-banner"><span class="summary-label">전체 줄거리 요약</span><p>${topSummary.filter(Boolean).join("&nbsp;&nbsp;·&nbsp;&nbsp;")}</p></div>`
    : "";
  return `<!DOCTYPE html>
<html lang="ko">
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    body{font-family:"Apple SD Gothic Neo","Malgun Gothic","Noto Sans KR",sans-serif;background:#f8f9fa;color:#1a1a2e}
    ...
    .scene-card{background:white;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.06);page-break-inside:avoid}
    ...
    @media print{body{background:white}.cover{min-height:100svh}.content{padding:40px 30px}.scene-card{box-shadow:none}@page{margin:0;size:A4 portrait}.scenes-grid{grid-template-columns:1fr 1fr;gap:20px}}
  </style>
</head>
<body>
  <div class="cover">
...
function ApiKeyPanel({ apiKey, onChange }: { apiKey: string; onChange: (v: string) => void }) {
  ...
  const handleSave = () => {
    if (typeof window !== "undefined") localStorage.setItem(STORAGE_KEY, apiKey);
    setSaved(true);
  ...
  return (
    <div className="rounded-2xl border border-amber-200 bg-amber-50/60 p-5 shadow-sm">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h2 className="text-sm font-semibold text-amber-800">Gemini API 키 설정</h2>
      </div>
      <p className="mb-3 text-xs leading-5 text-amber-700">
        이미지 생성에는 Google Gemini API 키가 필요합니다.{" "}
        <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="font-medium underline hover:text-amber-900">Google AI Studio</a>
        에서 무료로 발급받을 수 있습니다. 키는 이 브라우저에만 저장되며 서버에 보관되지 않습니다.
      </p>
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          />
          <button type="button" onClick={() => setShow(v => !v)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
            {show ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
          </button>
        </div>
        <button onClick={handleSave} disabled={!apiKey.trim()} className="rounded-lg bg-amber-500 px-4 py-2 text-sm font-medium text-white transition hover:bg-amber-600 disabled:cursor-not-allowed disabled:opacity-50">
          {saved ? "저장됨 ✓" : "저장"}
        </button>
        {apiKey && (
          <button onClick={handleClear} className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-600 transition hover:bg-gray-100">지우기</button>
        )}
      </div>
      {apiKey && <p className="mt-2 text-xs text-amber-600">✓ API 키가 입력되었습니다. 저장 버튼을 누르면 다음 방문에도 유지됩니다.</p>}
    </div>
  );
}
...
function SynopsisCard({ sections, topSummary }: { sections: SynopsisSections; topSummary: string[] }) {
  ...
  if (!hasContent && topSummary.every(s => !s)) {
    return <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-8 text-center text-sm text-gray-400">줄거리를 생성하면 여기에 정리된 시놉시스가 표시됩니다.</div>;
  }
  return (
    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      {topSummary.some(s => s) && (
        <div className="border-b border-indigo-100 bg-indigo-50/60 px-8 py-5">
          <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-indigo-400">전체 줄거리 요약</p>
          <p className="text-sm leading-8 text-gray-700">{topSummary.filter(Boolean).join("  ·  ")}</p>
        </div>
      )}
        ...
        {synopsisSectionMeta.map(({ label, key }) => {
          ...
          return (
            <div key={key} className="flex gap-4 px-8 py-5">
              <span className="mt-0.5 w-32 shrink-0 text-xs font-semibold uppercase tracking-wider text-indigo-500">{label}</span>
...
function CustomStylePanel({ prompt, onPromptChange, referenceImage, onImageChange, onImageRemove }: {
  prompt: string; onPromptChange: (v: string) => void;
  referenceImage: ReferenceImage | null; onImageChange: (img: ReferenceImage) => void; onImageRemove: () => void;
}) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-violet-200 bg-violet-50/50 p-4 space-y-4">
      <div>
        <label className="mb-1.5 block text-xs font-semibold text-violet-700">커스텀 스타일 프롬프트 <span className="font-normal text-gray-500">(이미지 생성 시 프롬프트 앞에 자동 삽입됩니다)</span></label>
        <textarea rows={3} placeholder="예) Ghibli-style soft watercolor, dreamy pastel backgrounds, hand-drawn characters" value={prompt} onChange={e => onPromptChange(e.target.value)}
        ...
        <p className="mt-1 text-xs text-gray-400">💡 영문으로 작성하면 이미지 품질이 높아집니다.</p>
      </div>
      <div>
        <label className="mb-1.5 block text-xs font-semibold text-violet-700">참조 이미지 <span className="font-normal text-gray-500">(선택 · 업로드하면 해당 이미지의 스타일을 참고해 생성합니다)</span></label>
        {referenceImage ? (
          <div className="relative inline-block">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={referenceImage.previewUrl} alt="참조 이미지" className="h-36 w-auto rounded-lg border border-violet-200 object-cover shadow-sm" />
            <button onClick={onImageRemove} className="absolute -right-2 -top-2 flex h-6 w-6 items-center justify-center rounded-full bg-gray-700 text-white shadow hover:bg-gray-900 transition"><X className="h-3.5 w-3.5" /></button>
          </div>
        ) : (
          <div onDragOver={e => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)}
            ...
            className={`flex h-28 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed transition select-none ${isDragging ? "border-violet-400 bg-violet-100" : "border-violet-200 bg-white hover:border-violet-400 hover:bg-violet-50"}`}>
...
// ─── SceneRegenPanel — 씬별 재생성 설정 패널 ─────────────────────────────────

function SceneRegenPanel({ scene, override, onChange, onRegen, isLoading }: {
  scene: Scene;
  override: SceneOverride;
  onChange: (o: SceneOverride) => void;
  onRegen: () => void;
  isLoading: boolean;
}) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  ...
  const readFile = (file: File) => {
    ...
    reader.onload = e => {
      ...
      const [header, base64] = dataUrl.split(",");
      const mimeType = header.replace("data:", "").replace(";base64", "");
      onChange({ ...override, refImage: { base64, mimeType, previewUrl: dataUrl } });
    };
    reader.readAsDataURL(file);
  };
  ...
  return (
    <div className="mt-3 rounded-xl border border-blue-200 bg-blue-50/50 p-4 space-y-3">
      <p className="text-xs font-semibold text-blue-700">🎬 이 씬의 이미지 재생성 설정</p>

      {/* 프롬프트 편집 */}
      <div>
        <label className="mb-1 block text-xs font-medium text-gray-600">
          이미지 프롬프트 <span className="text-gray-400">(영문 권장)</span>
        </label>
        <textarea
          rows={3}
          value={override.prompt}
          onChange={e => onChange({ ...override, prompt: e.target.value })}
          placeholder="이 씬에서 생성할 이미지를 영문으로 묘사해 주세요."
          className="w-full rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm leading-6 text-gray-800 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-200 placeholder:text-gray-400"
        />
      </div>
      ...
      <div>
        <label className="mb-1 block text-xs font-medium text-gray-600">
          참조 이미지 <span className="text-gray-400">(선택 · 이 이미지의 스타일을 참고해 생성)</span>
        </label>
        {override.refImage ? (
          <div className="relative inline-block">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={override.refImage.previewUrl} alt="참조 이미지" className="h-28 w-auto rounded-lg border border-blue-200 object-cover shadow-sm" />
            <button
              onClick={() => onChange({ ...override, refImage: null })}
          ...
          <div
            ...
            onDrop={e => { e.preventDefault(); setIsDragging(false); const f = e.dataTransfer.files?.[0]; if (f) readFile(f); }}
            onClick={() => fileInputRef.current?.click()}
            className={`flex h-24 cursor-pointer flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-dashed transition select-none ${isDragging ? "border-blue-400 bg-blue-100" : "border-blue-200 bg-white hover:border-blue-400 hover:bg-blue-50"}`}
          >
            <ImagePlus className="h-5 w-5 text-blue-400" />
            <p className="text-xs text-gray-500">클릭하거나 드래그해서 참조 이미지 업로드</p>
          </div>
        )}
        <input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/webp" className="hidden"
          onChange={e => { const f = e.target.files?.[0]; if (f) readFile(f); e.target.value = ""; }} />
      ...
      <button
        onClick={onRegen}
        disabled={isLoading || !override.prompt.trim()}
...
export default function Home() {
  ...
  // 씬별 재생성 패널 상태
  const [openRegenPanel, setOpenRegenPanel] = useState<number | null>(null);
  ...
  // 씬이 새로 생성되면 override 초기화
  const initOverrides = (newScenes: Scene[]) => {
    const init: Record<number, SceneOverride> = {};
    newScenes.forEach((s, i) => { init[i] = { prompt: s.imagePrompt, refImage: null }; });
    setSceneOverrides(init);
    setOpenRegenPanel(null);
  };

  const canGenerate = useMemo(() => synopsis.trim().length > 0 && apiKey.trim().length > 0, [synopsis, apiKey]);
  ...
  const buildImageRequestBody = (
    ...
    const base: Record<string, unknown> = {
      ...
      scenePrompt: override?.prompt || scene.imagePrompt,
      imageStyle: style,
      aspectRatio,
      geminiApiKey: apiKey,
    };
    // 씬별 참조 이미지가 있으면 우선 적용
    if (override?.refImage) {
      base.customReferenceImageBase64 = override.refImage.base64;
      base.customReferenceImageMimeType = override.refImage.mimeType;
  ...
  const requestSceneImage = async (scene: Scene, index: number, style: ImageStyleValue, override?: SceneOverride) => {
    const res = await fetch("/api/regenerate-image", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(buildImageRequestBody(scene, index, style, override)),
    });
    ...
    try {
      ...
      for (let idx = 0; idx < nextScenes.length; idx += 1) {
        try {
          ...
          setScenes(prev => prev.map((s, si) => si === idx ? { ...s, imageUrl } : s));
        } catch (error) {
          imageErrors.push(`씬 ${idx + 1}: ${error instanceof Error ? error.message : "이미지 생성 실패"}`);
        } finally {
          setImageLoading(idx, false);
        }
      }
      if (imageErrors.length > 0) alert(`일부 이미지 생성에 실패했습니다.\n${imageErrors.slice(0, 3).join("\n")}`);
    } catch (error) {
  ...
  const handleEditText = (index: number) => {
    const nextText = window.prompt("장면 텍스트를 수정해 주세요.", scenes[index].description);
    if (!nextText) return;
    setScenes(prev => prev.map((s, i) => i === index ? { ...s, description: nextText } : s));
  };
  ...
  // 씬별 설정으로 재실행
  const handleRegenWithOverride = async (index: number) => {
    const override = sceneOverrides[index];
    if (!override) return;
    setImageLoading(index, true);
    setOpenRegenPanel(null);
    try {
      const imageUrl = await requestSceneImage(scenes[index], index, imageStyle, override);
      setScenes(prev => prev.map((s, i) => i === index ? { ...s, imageUrl } : s));
    } catch (error) {
      alert(error instanceof Error ? error.message : "이미지 재실행 중 오류가 발생했습니다.");
    } finally {
    ...
    <main className="min-h-screen bg-[#F8F9FA] px-6 py-10 text-gray-900">
      <div className="mx-auto w-full max-w-6xl space-y-6">
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <label className="mb-2 block text-sm font-medium text-gray-700">시놉시스 입력</label>
          <div className="mb-2 rounded-lg border border-indigo-100 bg-indigo-50/70 p-3 text-xs text-gray-700">
            ...
            <p className="mt-1 leading-6">로그라인, 세계관 / 배경, 주요 캐릭터, 스토리 구조 (도입-전개-결말), 핵심 갈등, 주제, 톤 & 스타일, 기획 의도</p>
          </div>
          <textarea className="h-64 w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm leading-6 outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200"
            placeholder={synopsisTemplate} value={synopsis} onChange={e => setSynopsis(e.target.value)} />

          {/* 이미지 스타일 */}
          <div className="mt-4">
            <label className="mb-1.5 block text-sm font-medium text-gray-700">이미지 스타일</label>
            <p className="mb-2 text-xs text-gray-500">선택한 스타일이 7개 씬 이미지 전체에 일관되게 적용됩니다.</p>
            <div className="relative inline-block">
              <select value={imageStyle} onChange={e => setImageStyle(e.target.value as ImageStyleValue)}
            ...
            {imageStyle === "custom" && (
              <CustomStylePanel prompt={customStylePrompt} onPromptChange={setCustomStylePrompt}
                referenceImage={customReferenceImage} onImageChange={setCustomReferenceImage}
          ...
          {/* 종횡비 */}
          <div className="mt-4">
            <label className="mb-1.5 block text-sm font-medium text-gray-700">이미지 종횡비</label>
            <p className="mb-2 text-xs text-gray-500">생성되는 이미지의 가로세로 비율을 선택합니다.</p>
            <div className="flex flex-wrap gap-2">
              {ASPECT_RATIO_OPTIONS.map(opt => (
                <button key={opt.value} type="button" onClick={() => setAspectRatio(opt.value)}
                  className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition ${aspectRatio === opt.value ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-gray-300 bg-white text-gray-600 hover:border-indigo-300 hover:bg-indigo-50/50"}`}>
                  {opt.label}
                </button>
              ))}
            </div>
          ...
          <div className="mt-5 flex items-center gap-2">
            ...
            {!apiKey.trim() && <p className="text-xs text-amber-600">⚠ API 키를 먼저 입력하고 저장해 주세요</p>}
          </div>
          {isGenerating && (
            <div className="mt-4 inline-flex items-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-sm text-indigo-700">
              <Loader2 className="h-4 w-4 animate-spin" />{generationStatus || "생성 중..."}
            </div>
          )}
        </section>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleExport} disabled={!canExport}
              ...
              <Printer className="h-4 w-4" />스토리보드 출력 / PDF
            </button>
          </div>
          ...
          {scenes.map((scene, index) => {
            ...
            const isPanelOpen = openRegenPanel === index;
            const override = sceneOverrides[index] ?? { prompt: scene.imagePrompt, refImage: null };

            return (
              <article key={`scene-${index}`} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className={`relative w-full overflow-hidden rounded-lg border border-gray-200 bg-gray-100 ${currentAspect?.aspect ?? "aspect-video"}`}>
                    ...
                    <Image src={scene.imageUrl} alt={`${scene.title} generated image`} fill unoptimized className="object-cover" />
                  </div>
                  ...
                  <div className="flex flex-col rounded-lg border border-gray-200 bg-gray-50 p-4">
                    ...
                    <p className="mt-2 flex-1 text-sm leading-6 text-gray-700">{scene.description || "-"}</p>
                    <div className="mt-4 flex flex-wrap items-center gap-2">
                      <button onClick={() => handleEditText(index)}
                        ...
                        <PenLine className="h-3.5 w-3.5" />텍스트 수정
                      </button>
                      <button onClick={() => handleRegenerateImage(index)} disabled={isImageLoading}
                        ...
                        {isImageLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}
                        이미지 재실행
                      </button>
                      {/* ⚙️ 씬별 설정 버튼 */}
                      <button
                        onClick={() => setOpenRegenPanel(isPanelOpen ? null : index)}
                        title="프롬프트/참조 이미지로 재생성"
                        className={`inline-flex items-center gap-1 rounded-md border px-2.5 py-1.5 text-xs font-medium transition ${isPanelOpen ? "border-blue-400 bg-blue-50 text-blue-700" : "border-gray-300 bg-white text-gray-700 hover:bg-gray-100"}`}>
                        <Settings2 className="h-3.5 w-3.5" />
                        {isPanelOpen ? "설정 닫기" : "고급 재생성"}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                {isPanelOpen && (
                  <SceneRegenPanel
                    scene={scene}
                    override={override}
                    onChange={o => setSceneOverrides(prev => ({ ...prev, [index]: o }))}
                    onRegen={() => handleRegenWithOverride(index)}
