...
/** English style prefix injected at the front of every image prompt */
const IMAGE_STYLE_PREFIXES: Record<ImageStyleValue, string> = {
  pixar:
...
export async function POST(req: Request) {
  try {
    ...
    // Resolve style prefix (default to live_action if not provided / invalid)
    const styleKey: ImageStyleValue =
      imageStyle && imageStyle in IMAGE_STYLE_PREFIXES
        ? imageStyle
        : "live_action";
    const stylePrefix = IMAGE_STYLE_PREFIXES[styleKey];
    ...
    // Style prefix is prepended to both the image prompt and the descriptive block
    const prompt = `
${stylePrefix}
